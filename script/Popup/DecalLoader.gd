...
func _physics_process(_delta):
	if get_parent().get_parent().get_parent().open == true and get_parent().get_parent().get_parent().open_window == "DecalLoader":
...
func _select(coord):
	...
	if id < 68:
		...
		if number < 64:
			...
			$Preview/Preview.texture = load("res://image/decal/decal" + fixed_number + ".png")
			$Preview/ID.text = str(id) + " (" + str(int(id) % 8) + "x" + str(int(int(id) / 8)) + ")"
			$Preview/Preview.scale = Vector2(1.152, 1.152)
			
			selected = id
		
		else:
			
			fixed_number = str(abs(number - 63))
			$Preview/Preview.texture = load("res://image/door/door" + str(fixed_number) + ".png")
			$Preview/ID.text = str(str(abs(number - 63) * -1) + " (door)")
			$Preview/Preview.scale = Vector2(0.288, 0.288)
		
			selected = abs(number - 63) * -1
